...
```bash
curl -fsS -o assets/js/lenis.min.js "https://unpkg.com/lenis@1.1.14/dist/lenis.min.js"
grep -q "globalThis.Lenis" assets/js/lenis.min.js && echo "lenis ok" || echo "LENIS BROKEN"
```

## 1.3 Декоративные картинки (6 шт.)

Пять вырезанных webp (салат, помидор, сыр, котлета, бургер) плюс `cta.webp` —
обычное прямоугольное фото для внутренней страницы:

```bash
for f in lettuce tomato cheese-logo meat burger cta; do
  curl -fsS -o "assets/img/$f.webp" "https://www.cravburgers.shop/img-webp/$f.webp" || echo "FAIL $f"
done
# у настоящего webp по смещению 8 стоит слово WEBP; страница-ошибка его не имеет
for f in assets/img/*.webp; do
  printf "%s %s %s\n" "$(dd if="$f" bs=1 skip=8 count=4 2>/dev/null)" "$(wc -c <"$f")" "$f"
done
```
...
im.crop(box).save("assets/img/cherry-burger.png", optimize=True)
print("hero:", im.crop(box).size)   # ожидается примерно 769x541, файл ~1 МБ
PY
```
...
У 64 фото фон уже прозрачный — они просто ужимаются. У остальных 12 (лимонады,
салаты, бульон, чиа-пудинг) фон залит цветом, и его надо вырезать.

Наивная заливка от краёв на них не работает: под посудой лежит мягкая тень,
а на снимках салатов ещё и фактура тёмного стола — они цепляются к блюду
тонкими перемычками и остаются висеть грязными пятнами, что особенно заметно
на красном фоне секции меню. Поэтому после заливки делается размыкание
с реконструкцией: маска сжимается (перемычки рвутся), из получившихся кусков
берётся тот, который сильнее всего отличается от фона по цвету — это и есть
блюдо, — а затем форма разжимается обратно и обрезается по исходному силуэту,
чтобы не потерять края. Допуск и радиус сжатия подобраны для каждого фото
отдельно и лежат в словаре `CUTOUT`.
...
# фото, у которых фон на исходнике не вырезан: slug -> (допуск, радиус эрозии)
CUTOUT = {
    "bulion-de-pui": (50, 2), "cherry-kiwi": (42, 3), "chicken-crispy-salad": (58, 5),
    "classic-lemonade": (18, 3), "mojito": (42, 3), "pina-colada": (18, 3),
    "strawberry-basil": (38, 2), "chia-pudding": (56, 6), "chicken-torro-salad": (62, 5),
    "crispy-fish-salad": (62, 5), "shrimp-salad": (62, 5), "vegetarian-torro-salad": (66, 5),
}
...
def fetch(url):
    if url not in _cache:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        _cache[url] = Image.open(io.BytesIO(urllib.request.urlopen(req, timeout=60).read())).convert("RGBA")
    return _cache[url].copy()

def pick_blob(mask, weight, W, H):
    # связная область, у которой сумма непохожести на фон максимальна
    mp = mask.load()
    seen = bytearray(W * H)
    best, best_score = [], -1
    for sy in range(H):
        for sx in range(W):
            if seen[sy*W+sx] or mp[sx, sy] == 0:
                continue
            comp = []; q = deque([(sx, sy)]); seen[sy*W+sx] = 1; total = 0
            while q:
                x, y = q.popleft(); comp.append((x, y)); total += weight[y*W+x]
                for nx, ny in ((x+1,y), (x-1,y), (x,y+1), (x,y-1)):
                    if 0 <= nx < W and 0 <= ny < H and not seen[ny*W+nx] and mp[nx, ny]:
                        seen[ny*W+nx] = 1; q.append((nx, ny))
            if total > best_score:
                best_score, best = total, comp
    return best

def cut_background(im, tol, erode):
    # Заливка фона от краёв + размыкание с реконструкцией: эрозия рвёт тонкие
    # перемычки, которыми к блюду цепляются остатки тени и фактуры стола,
    # затем форма восстанавливается обратно по исходному силуэту.
    W, H = im.size
    px = im.load()
    corners = [px[2, 2], px[W-3, 2], px[2, H-3], px[W-3, H-3]]
    bg = tuple(sum(c[i] for c in corners) // 4 for i in range(3))

    weight = bytearray(W * H)          # насколько пиксель не похож на фон
    for y in range(H):
        for x in range(W):
            p = px[x, y]
            d = (abs(p[0]-bg[0]) + abs(p[1]-bg[1]) + abs(p[2]-bg[2])) // 3
            weight[y*W+x] = 255 if d > 255 else d

    isbg = bytearray(W * H); q = deque()
    def push(x, y):
        i = y*W+x
        p = px[x, y]
        if not isbg[i] and abs(p[0]-bg[0]) + abs(p[1]-bg[1]) + abs(p[2]-bg[2]) <= tol * 3:
            isbg[i] = 1; q.append((x, y))
    for x in range(W):
        push(x, 0); push(x, H-1)
    for y in range(H):
        push(0, y); push(W-1, y)
    ...
    core = silh.filter(ImageFilter.MinFilter(erode*2+1))        # рвём перемычки
    keep = Image.new("L", (W, H), 0)
    kp = keep.load()
    for x, y in pick_blob(core, weight, W, H):
        kp[x, y] = 255
    keep = keep.filter(ImageFilter.MaxFilter(erode*2+1))        # растим обратно
    kp = keep.load()
    alpha = Image.new("L", (W, H), 0)
    ap = alpha.load()
    for y in range(H):                                          # обрезаем по силуэту
        for x in range(W):
            if kp[x, y] and sp[x, y]:
                ap[x, y] = 255
    im.putalpha(alpha.filter(ImageFilter.GaussianBlur(1.0)))
